...
static int send_hid_report_init(uint8_t report_id) {
  static struct gamepad_report_t gamepad
      __attribute__((aligned(64))) = {.report_id = 1,
                                      .buttons = 0,
                                      .left_x = 0,
                                      .left_y = 0,
                                      .right_x = 0,
...
static void fill_gamepad_report(const SceCtrlData *pad,
  ...
  gamepad->ltrigger = (int8_t)(ltrig);
  gamepad->rtrigger = (int8_t)(rtrig);

  /*uint16_t x_mid = (back_info.maxAaX - back_info.minAaX) / 2 + back_info.minAaX;
  uint16_t y_range = (back_info.maxAaY - back_info.minAaY) / 2;
  uint16_t y_mid = y_range + back_info.minAaY;
  for (int i = 0; i < touch->reportNum; i++)
  {
	  int8_t trig_val = 0;
	  if (touch->report[i].y < y_range + back_info.minAaY){
	  	trig_val = (int8_t)((uint32_t)(y_range - (touch->report[i].y - back_info.minAaY)) * 127 / y_range);
	  }

	  if (touch->report[i].x < x_mid && gamepad->ltrigger == 0)
		  gamepad->ltrigger = trig_val;
	  if (touch->report[i].x >= x_mid && gamepad->rtrigger == 0)
		  gamepad->rtrigger = trig_val;
  }*/
}
...
static int vitastick_udcd_process_request(int recipient, int arg,
  ...
  LOG("request: %x type: %x wValue: %x wIndex: %x wLength: %x\n", req->bRequest,
      req->bmRequestType, req->wValue, req->wIndex, req->wLength);
...
static int vitastick_udcd_change_setting(int interfaceNumber,
                                         int alternateSetting, int bus) {
...
static void vitastick_udcd_configure(int usb_version, int desc_count,
                                     SceUdcdInterfaceSettings *settings,
                                     void *user_data) {
  LOG("vitastick_udcd_configure %d %d %p %d\n", usb_version, desc_count,
      settings, settings->numDescriptors);
}
...
static SceUdcdDriver vitastick_udcd_driver = {VITASTICK_DRIVER_NAME,
                                              2,
                                              &endpoints[0],
                                              &interfaces[0],
                                              &devdesc_hi,
 ...
 */
int vitastick_uploadtouchdata(int8_t ltrigger, int8_t rtrigger){
	unsigned long state;
	ENTER_SYSCALL(state);
	ltrig = ltrigger;
	rtrig = rtrigger;
...
int module_start(SceSize argc, const void *args) {
  ...
  usb_event_flag_id =
      ksceKernelCreateEventFlag("vitastick_event_flag", 0, 0, NULL);
